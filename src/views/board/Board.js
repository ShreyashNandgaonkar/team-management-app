...
function Board(props) {
  ...
  const getColumnData = () => {
    ...
    Axios.get(
      `https://team-management-app-62109.firebaseio.com/boardContents/${props.location.state.boardId}/column.json`
    )
      .then((res) => {
  ...
  const handleColumnDelete = (columnId, e) => {
    Axios.delete(
      `https://team-management-app-62109.firebaseio.com/boardContents/${props.location.state.boardId}/column/${columnId}.json`
    )
      .then((res) => {
  ...
  const handleBoardDelete = (e) => {
    if (window.confirm("Are you sure you want to delete the board?")) {
      Axios.delete(
        `https://team-management-app-62109.firebaseio.com/boardContents/${props.location.state.boardId}.json`
      )
        .then((res) => {
          history.push("/");
        })
  ...
  const handleCardDrop = (droppedColumnId, e) => {
    ...
    if (draggedCardData !== null) {
      ...
      Axios.delete(
        `https://team-management-app-62109.firebaseio.com/boardContents/${props.location.state.boardId}/column/${prevColId}/card/${prevCardId}.json`
      )
        .then((res) => {
      ...
      Axios.post(
        `https://team-management-app-62109.firebaseio.com/boardContents/${props.location.state.boardId}/column/${droppedColumnId}/card.json`,
        {
